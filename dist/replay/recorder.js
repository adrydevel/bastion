...
export class ReplayDeck {
    ...
    for(agent) {
        return {
            reason: async (system, user) => {
                const hit = this.exchanges.find((e) => e.agent === agent && e.system === system && e.user === user);
                if (!hit) {
                    throw new Error(`no recorded exchange for "${agent}" — this cassette was produced by a different pipeline`);
                }
                return hit.response;
            },
        };
