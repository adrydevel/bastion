...
import { buildCassette, inputHashOf, type Cassette } from "./cassette.js";
...
function fixedWindow(ticker = "NVDA"): StockTokenQuote[] {
...
async function record(): Promise<Cassette> {
...
describe("replay", () => {
  ...
  it("fails when a model response was rewritten", async () => {
    const cassette = await record();
    const exchanges = cassette.exchanges.map((e, i) =>
      i === 0 ? { ...e, response: "short, confidence: 0.99" } : e,
    );
  ...
  it("fails when the market window was swapped underneath the proof", async () => {
    const cassette = await record();
    const quotes = cassette.input.quotes.map((q, i) =>
      i === 0 ? { ...q, price: q.price * 1.5 } : q,
    );
